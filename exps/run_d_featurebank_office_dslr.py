...
sys.argv = ["run_d_featurebank_office_dslr"]
args = args_parser()
...
print("=" * 78)
print(f"D + FEATUREBANK MECHANISM ORACLE -- M={_M}, D arm, seed={args.seed}")
print(f"domain_keyed_proto = {args.domain_keyed_proto}   "
      ...
      f"lambda_G = {LAMBDA_G}")
print(f"feature_bank_domains = {FEATURE_BANK_DOMAINS}   lambda_FB = {LAMBDA_FB}")
print("-" * 78)
print("reference to beat (M4_D_seed1, same seed/config, no FeatureBank -- l2_layer2_consumption/"
      "seed1_acc.csv, reused not re-run): "
      "X-dom-best=0.3790 (r66)  own-peak=0.4355 (r14)  late-window=0.3669 (r90-99)")
print("(seed0's own FeatureBank result, for context only, NOT this run's comparison point: "
      "X_FB=0.3468 vs D-control-seed0 0.3387, R_D=0.018 -- weak)")
print("=" * 78)

RESUME_INPUT_DIR = None
RESUME = False

_tag = "d_featurebank_M4_seed1"

if os.path.isdir("/kaggle/working"):
    drive_base = "/kaggle/working/FedPall/limitation_2/d_featurebank_dslr"
elif os.path.isdir("/content/drive/MyDrive"):
    drive_base = "/content/drive/MyDrive/FedPall/limitation_2/d_featurebank_dslr"
else:
    raise RuntimeError(
...
if RESUME_INPUT_DIR is not None:
    ...
    if os.path.isdir(_resume_weights):
        ...
        print("Restored weights_seed0/ from RESUME_INPUT_DIR -- preserves the true all-round best "
              "checkpoint weights even if no round after the resume point beats best_acc.")
    else:
...
print(f"  -> dslr X-dom-best = {accuracy_list['dslr'][xdom_best_round]:.4f}   "
      f"(reference M4_D_seed1 = 0.3790 -- compare against THIS, not seed0's 0.3387)")
print(f"  -> dslr own-peak    = {max(accuracy_list['dslr']):.4f}   "
      f"(reference M4_D_seed1 = 0.4355, secondary metric)")
print(f"  -> delta_FB^(1) (X-dom-best, paired vs D-control seed1) = "
      f"{accuracy_list['dslr'][xdom_best_round] - 0.3790:.4f}")
print("-" * 78)
