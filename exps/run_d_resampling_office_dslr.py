...
sys.argv = ["run_d_resampling_office_dslr"]
args = args_parser()
...
print("=" * 78)
print(f"D + RESAMPLING BRIDGE ORACLE -- M={_M}, D arm, seed={args.seed}")
print(f"domain_keyed_proto = {args.domain_keyed_proto}   "
      ...
      f"lambda_G = {LAMBDA_G}")
print("-" * 78)
print("resampled clients (static reads only, no draws -- see RNG NOTE in "
      "run_information_oracle_office_dslr.py):")
...
print("reference to beat (M4_D_seed0, same seed/config, no resampling): "
...
if RESUME or RESUME_INPUT_DIR is not None:
    print("WARNING: resuming this driver restarts the 4 dslr clients' resampling generators from their "
          "own round-0 draws (generator state isn't checkpointed) -- see "
          "run_information_oracle_office_dslr.py's RESUME CAVEAT comment for the full explanation. "
          "The run is still valid, just not a single unbroken 100-round stream for those 4 clients.")

_tag = f"d_resampling_M4_seed{args.seed}"

if os.path.isdir("/kaggle/working"):
    drive_base = "/kaggle/working/FedPall/limitation_2/d_resampling_dslr"
    ...
    drive_base = "/content/drive/MyDrive/FedPall/limitation_2/d_resampling_dslr"
else:
    raise RuntimeError(
...
print(f"  -> dslr X-dom-best = {accuracy_list['dslr'][xdom_best_round]:.4f}   "
      f"(reference M4_D_seed0 = 0.3387, NOT the baseline oracle's 0.8145)")
