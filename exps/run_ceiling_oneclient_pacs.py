...
sys.argv = ["run_ceiling_oneclient_pacs"]
args = args_parser()
...
DOMAIN_CLIENT_OVERRIDES = {"photo": 1, "art_painting": 1, "cartoon": 1, "sketch": 1}
train_loader_list, client_domains, test_loader_list = prepare_data_PACS_multi_clients(
    args, domain_client_overrides=DOMAIN_CLIENT_OVERRIDES
)
args.num_users = len(train_loader_list)

print("=" * 78)
print(f"CEILING (one client per domain, plain FedPall, protocol-matched reconstruction) -- PACS -- seed={args.seed}")
...
_tag = f"ceiling_oneclient_pacs_seed{args.seed}"

if os.path.isdir("/kaggle/working"):
    drive_base = "/kaggle/working/FedPall/limitation_2/ceiling_oneclient_pacs"
elif os.path.isdir("/content/drive/MyDrive"):
    drive_base = "/content/drive/MyDrive/FedPall/limitation_2/ceiling_oneclient_pacs"
else:
    raise RuntimeError(
...
for domain in datasets_name:
    print(f"    {domain:<12} @X-dom-best round = {accuracy_list[domain][xdom_best_round]:.4f}")
...
for domain in datasets_name:
    print(f"    {domain:<12} = {late_avg_by_domain[domain]:.4f}")
