...
sys.argv = ["run_b_server_expand_office_dslr"]
args = args_parser()
...
for i, dom in enumerate(client_domains):
    ...
    feature_loader_list[i] = DataLoader(
        ds,
    ...
    _oracle_report.append((i, len(ds), n_i, args.batch, len(feature_loader_list[i])))

if not _oracle_report:
    raise RuntimeError(f"No '{ORACLE_DOMAIN}' clients found -- client_domains={client_domains}")

print("=" * 78)
print(f"RESAMPLING PATH DECOMPOSITION -- ARM B (fixed local SGD, expanded feature/upload), M={_M}, seed={args.seed}")
print(f"domain_keyed_proto = {args.domain_keyed_proto}   "
      ...
      f"lambda_G = {LAMBDA_G}")
print("-" * 78)
print("dslr clients (train_loader=UNCHANGED fixed original, feature_loader=resampled):")
...
_tag = f"b_server_expand_M4_seed{args.seed}"

if os.path.isdir("/kaggle/working"):
    drive_base = "/kaggle/working/FedPall/limitation_2/b_server_expand_dslr"
elif os.path.isdir("/content/drive/MyDrive"):
    drive_base = "/content/drive/MyDrive/FedPall/limitation_2/b_server_expand_dslr"
else:
    raise RuntimeError(
...
if RESUME_INPUT_DIR is not None:
    ...
    if os.path.isdir(_resume_weights):
        ...
        print("Resume requested: feature_loader generator state will be restored from the checkpoint.")
...
print(f"  -> dslr X-dom-best (ARM B) = {_x_b:.4f}   (D=0.3387, matched Resampling ceiling=0.7823)")
print(f"  -> dslr own-peak            = {max(accuracy_list['dslr']):.4f}   (D own-peak=0.3790)")
print(f"  -> position in [D, Resampling] span: {(_x_b - 0.3387)/(0.7823 - 0.3387):.4f}   "
      f"(0=no recovery, 1=matches full Resampling)")
print("     (late-window still to be computed from acc.csv rounds 90-99, then I_X = X_R - X_A - X_B + X_D once A exists)")
print("-" * 78)
