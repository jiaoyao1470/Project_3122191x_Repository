...
if len(sys.argv) not in (2, 3) or sys.argv[1] not in ("baseline", "mixing"):
    raise RuntimeError(
        "Usage: python run_d_peer_update_mixing_office_dslr.py <baseline|mixing> [alpha]"
    )
_condition = sys.argv[1]
_use_mixing = (_condition == "mixing")
_alpha_arg = float(sys.argv[2]) if len(sys.argv) == 3 else 0.10
sys.argv = ["run_d_peer_update_mixing_office_dslr"]
args = args_parser()
...
print("=" * 78)
_mix_domains_repr = "{'dslr'}" if _use_mixing else "None"
print(f"D-MATCHED vs D+SAME-DOMAIN ENCODER UPDATE MIXING -- condition={_condition}  "
      f"(peer_update_mixing_domains={_mix_domains_repr})  M={_M}, seed={args.seed}")
...
if _use_mixing:
    print(f"peer_update_mixing_alpha = {PEER_MIX_ALPHA}")
print("-" * 78)
print("dslr clients (fixed original partitions, no resampling):")
...
print("  MeanDispersion         : separate run, tracked independently (repair candidate #1)")
print("=" * 78)
...
if _condition == "baseline":
    _tag = "d_meandispersion_M4_seed0_baseline_v2calib"
else:
    _tag = f"d_peer_update_mixing_M4_seed0_mixing_alpha{int(round(PEER_MIX_ALPHA * 100)):03d}"
...
train_loss, accuracy_list, datasets_name = ours(
    ...
    use_mean_dispersion=False,
    peer_update_mixing_domains=({"dslr"} if _use_mixing else None),
    peer_update_mixing_alpha=(PEER_MIX_ALPHA if _use_mixing else 0.0),
)
...
print("     (run BOTH conditions, then: R_mix = (X_mixing - X_baseline) / (0.7500 - X_baseline), "
      "R_mix_late = (L_mixing - L_baseline) / (0.7685 - L_baseline))")
print("-" * 78)
