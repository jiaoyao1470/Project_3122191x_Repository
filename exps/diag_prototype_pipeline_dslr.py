...
if len(sys.argv) < 6 or sys.argv[1] not in ("1", "2", "4") or sys.argv[2] not in ("baseline", "D"):
    raise SystemExit(
        "Usage: diag_prototype_pipeline_dslr.py <1|2|4> <baseline|D> <checkpoint_path> "
        "<M1_checkpoint_path_same_condition> <output_prefix>"
    )
_M = int(sys.argv[1])
_condition = sys.argv[2]
_checkpoint_path_arg = sys.argv[3]
_m1_checkpoint_path_arg = sys.argv[4]
_output_prefix = sys.argv[5]
sys.argv = ["diag_prototype_pipeline_dslr"]
args = args_parser()
...
G_k_real = None
if _condition == "D" and checkpoint.get("prev_G_state") is not None:
    G_k_real = checkpoint["prev_G_state"]
    print(f"Found real trained G_k in checkpoint (classes present: {sorted(G_k_real.keys())})")
elif _condition == "D":
    print("condition=D but no prev_G_state in checkpoint (cold start or Layer2 never triggered)")
...
csv_path = f"{_output_prefix}_M{_M}_{_condition}_prototype_diag.csv"
...
tensor_path = f"{_output_prefix}_M{_M}_{_condition}_prototype_tensors.pt"
torch.save({
    ...
    "R_dk_M1_reference": {c: v.cpu() for c, v in R_dk.items()},
