...
sys.argv = ["run_d_stronglocalaug_alldegraded_office"]
args = args_parser()
...
print("=" * 78)
print(f"D + STRONG LOCAL AUGMENTATION, ALL-DEGRADED-DOMAINS GENERALITY TEST -- caltech+amazon+dslr "
      f"train_loaders get richer views, feature_loaders fixed, webcam untouched (control), M(dslr)={_M}, "
      f"seed={args.seed}")
...
print("untouched clients (webcam control, feature_loader_list entry = None -> falls back to their own "
...
print("reference points (D_matched seed0, robust-standard, reconstructed via reconstruct_robust_standard.py "
      "on d_meandispersion_M4_seed0_baseline_v2calib_acc.csv -- the correct 'before' for ALL FOUR domains):")
print("    caltech    robust=40.59%   late-window=39.66%   (ceiling=44.44% even UNFRAGMENTED -- see header,")
print("               weight this domain's result separately from amazon's)")
print("    amazon     robust=64.58%   late-window=63.35%   (ceiling=73.78% -- cleanest generality test case)")
print("    dslr       robust=39.52%   late-window=30.18%   (ceiling=74.19%; X-dom-best/late-window elsewhere")
print("               quoted as 37.90%/28.95% is a DIFFERENT selection methodology, do not mix)")
print("    webcam     robust=75.86%   late-window=72.38%   (ceiling=74.14% -- untouched control, expect ~no change)")
print("    robust mean across 4 domains (D_matched) = 55.14%   |   ceiling(3-seed) = 66.64%   |   "
      "baseline(3-seed,fragmented,no repair) = 52.11%   |   D(3-seed,full repair) = 55.13%")
print("Once this run finishes, pass its own acc.csv through reconstruct_robust_standard.py for the "
      "apples-to-apples comparison -- do NOT use this driver's own X-dom-best/late-window printout below "
      "for that headline comparison (different, round-selection-based methodology).")
print("=" * 78)
...
_tag = f"d_stronglocalaug_alldegraded_M4_seed{args.seed}"

if os.path.isdir("/kaggle/working"):
    drive_base = "/kaggle/working/FedPall/limitation_2/d_stronglocalaug_alldegraded"
elif os.path.isdir("/content/drive/MyDrive"):
    drive_base = "/content/drive/MyDrive/FedPall/limitation_2/d_stronglocalaug_alldegraded"
else:
    raise RuntimeError(
...
print("Run reconstruct_robust_standard.py on this run's acc.csv for the apples-to-apples robust-standard "
      "comparison against D_matched(55.14%)/D(55.13%,3-seed)/baseline(52.11%,3-seed)/ceiling(66.64%,3-seed).")
print("-" * 78)
